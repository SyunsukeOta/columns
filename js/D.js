...
class Board {
  ...
  checkJewels(checkCells) {
    ...
    for(let j=0; j<checkCells.length; j++) {
      ...
      for(let i=-1; i<=1; i++) {
        ...
        let baseJewel = this.jewels[baseTop][baseLeft];
        let leftJewel = this.jewels[baseTop][baseLeft-1];
        let rightJewel = this.jewels[baseTop][baseLeft+1];
  ...
  fallCell(cell) {
    ...
    for(let i=cell.y-1; i>=0; i--) {
      this.jewels[i+1][cell.x] = this.jewels[i][cell.x];
      if (this.jewels[i+1][cell.x] !== null) {
        fallCells.push({x: cell.x, y: i+1});
      }
    }
    return fallCells;
  }
