class GameContainer {
  constructor() {
    ...
    this.scoreLabel = new Label(this, 'Score', 300, 250);
    this.plusScoreLabel = new Label(this, 'plusScore', 300, 200);
    this.level = 1;
    this.levelLabel = new Label(this, 'Level', 300, 300);
    this.deleteJewelsLabel = new Label(this, 'Jewel', 300, 350);
    this.highScoreLabel = new Label(this, 'HighScore', 300, 400);
    this.deleteJewels = 0;
    this.highScore = 0;
    this.levelBase = 20; //何個ジュエルを消したらレベルが上がるか
    this.speedMax = 5;
    this.speedUpLevel = 5; //何レベル上がったら、スピードが上がるか
  }
  createGraphics() {
  ...
  addSpaceKeyEvent(callback) {
    ...
    document.addEventListener("keydown", e => {
      if (e.keyCode == 32 && this.keyEnabled) {
        callback();
        e.preventDefault();
        return;
      }
  ...
  addRightKeyEvent(callback) {
    document.addEventListener("keydown", e => {
      if (e.keyCode == 39 && this.keyEnabled) {
        callback();
        e.preventDefault();
        return;
      }
  ...
  addLeftKeyEvent(callback) {
    document.addEventListener("keydown", e => {
      if (e.keyCode == 37 && this.keyEnabled) {
        callback();
        e.preventDefault();
        return;
      }
  ...
  addDownKeyEvent(callback) {
    document.addEventListener("keydown", e => {
      if (e.keyCode == 40 && this.keyEnabled) {
        callback();
        e.preventDefault();
        return;
      }
  ...
  animate(){
    ...
    if (this.frameCount >= 120 / this.speed) {
      this.board.moveDown();
      this.frameCount = 0;
    } else {
      this.frameCount++;
  ...
  start() {
    this.board.startBlock();
    this.board.drawJewels();
    this.animate();
    this.drawScore();
    this.drawPlusScore(0,0);
    this.drawLevel();
    this.drawDeleteJewels();
    this.drawHighScore();
  }
  gameOver() {
    alert('gameover');
    this.board.resetJewels();
    this.highScore = Math.max(this.score, this.highScore);
    this.score = 0;
    this.deleteJewels = 0;
    this.level = 1;
    this.board.nextBlock.clearJewels();
    this.board.nextBlock = null;
    this.board.startBlock();
    this.drawScore();
    this.drawPlusScore(0,0);
    this.drawDeleteJewels();
  ...
  drawScore() {
    this.scoreLabel.draw(this.score);
  }
  drawPlusScore(jewelCount, combo) {
    this.plusScoreLabel.draw(this.calcScore(jewelCount, combo));
  }
  drawLevel() {
    this.levelLabel.draw(this.level);
  }
  drawDeleteJewels() {
    this.deleteJewelsLabel.draw(this.deleteJewels);
  }
  drawHighScore() {
    this.highScoreLabel.draw(this.highScore);
  }
  calcScore(jewelCount, combo) {
    return jewelCount * combo;
  }
  addScore(jewelCount, combo) {
    this.score += this.calcScore(jewelCount, combo);
    this.deleteJewels += jewelCount;
    this.level = Math.ceil((this.deleteJewels+1)/this.levelBase);
    this.speed = Math.ceil((this.level+1)/this.speedUpLevel);
    ...
    this.drawScore();
    this.drawPlusScore(jewelCount,combo);
    this.drawDeleteJewels();
    this.drawLevel();
  }
}
...
class Label {
  constructor(gameContainer, title, x, y) {
    ...
    this.title = title;
    gameContainer.addGraphicChild(this.text);
  }
  draw(message) {
    this.text.text = `${this.title}: ${message}`;
  }
}
...
class Jewel {
  ...
  draw(left, top) {
    if (this.jewel === null) {
      ...
      this.jewel.drawRect(1, 1, this.jewelSize-2, this.jewelSize-2);
      this.jewel.endFill();
...
class Board {
  ...
  startBlock() {
    ...
    this.nextBlock = new Block(this.gameContainer);
    this.drawNextBlock();
  }
  moveRight() {
  ...
  async checkJewels(checkCells, combo) {
    if (!combo) {
      combo = 1;
    }
    console.log('*** checkCells *** ' + combo);
    ...
    for(let i=0; i<deleteCells.length; i++) {
      if(this.deleteCell(deleteCells[i])) {
        deleteCellCount++;
      }
    }
    ...
    this.printJewels();
    this.gameContainer.addScore(deleteCellCount, combo);
    //console.log(this.jewels);
    if (fallCells.length) {
      this.checkJewels(fallCells, combo + 1);
    }
  }
  getJewel(left, top) {
  ...
  deleteCell(cell) {
    if (!this.jewels[cell.y][cell.x]) return false;
  ...
  checkGameover() {
    for(let i=0; i<this.gameContainer.boardCellWidth; i++) {
      if (this.jewels[i==this.gameContainer.startCellLeft?1:0][i] !== null) {
        return true;
      }
    }
    return false;
  ...
  freezeBlock() {
    ...
    for(let i=0; i<3; i++) {
      if (this.jewels[this.cellTop+i][this.cellLeft] === null) {
